...
T <- as.integer(100) #length of data file
D <- matrix(vector(mode= "integer", length = T*2), ncol = 2) # y
...
for (p in 1:2) {
  D[,p] <- ifelse((t<c), t*e,( c*e + (t-c)*l)) + rnorm(100, 0, sigma)
}
...
MarginalChangePoint <- stan(file = 'Model_changePointContinuous2.stan', 
                            iter = 10, chains = 4)
save(MarginalChangePoint, file="Fit/MarginalChangePoint.Rdata")
```