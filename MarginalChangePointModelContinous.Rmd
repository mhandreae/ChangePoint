...
We generate the data in long format and run *rstan* in logn format as suggested in the stan manual under **12.1**
...
# generate a patient ID
 id[1:z[1]] <- 1
 id[(sum(z[i-1:i]) +1) : (sum(z[i-1:i]) + z[i])] <-i 
...
# Save data for stan
continuous_data7 <- list(r_a=100, # intercept prior
                         r_e = 0, # early slope prior
...
g +geom_path(aes(observed,D, group=id, color=id)) +
...
MarginalChangePoint6 <- stan(file = 'Model_changePointContinuous7.stan', 
